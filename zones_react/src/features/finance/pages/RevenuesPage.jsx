...
function ChartTooltip({ active, payload, label, isLight }) {
  ...
  return (
    <div style={boxStyle}>
      ...
      <TooltipRow value={value} mutedColor={isLight ? "#4b5563" : "#94a3b8"} />
    </div>
  );
}

function TooltipRow({ value, mutedColor }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem" }}>
      <span style={{ color: mutedColor }}>المبلغ</span>
...
export default function RevenuesPage() {
  ...
  const revClass =
    totals.revDelta > 0.4
      ? "finance-card__delta--up"
      : totals.revDelta < -0.4
        ? "finance-card__delta--down"
        : "finance-card__delta--flat";
